...
export function normalizeGitUrl(input: string): string {
  ...
  if (url.startsWith("https://")) url = url.slice("https://".length)
  else if (url.startsWith("http://")) url = url.slice("http://".length)
  if (!url.includes(".") && url.split("/").length === 2) url = `github.com/${url}`
 ...
 */
export function extractDirName(input: string): string {
...
export async function countSkills(dir: string): Promise<number> {
  let count = 0
  const entries = await fs.promises.readdir(dir, { withFileTypes: true })

  if (entries.some((e) => e.isFile() && e.name === "SKILL.md")) count++
    ...
    if (fs.existsSync(sub)) {
      try {
        for (const s of await fs.promises.readdir(sub, { withFileTypes: true })) {
          if (s.isDirectory() && fs.existsSync(path.join(sub, s.name, "SKILL.md"))) count++
        }
      } catch { /* ignore */ }
    }
  }
 ...
 */
export async function symlinkToAgents(skillDirName: string): Promise<string[]> {
    ...
    try {
      ...
      try {
        ...
        if (stat.isSymbolicLink()) {
          ...
          if (existing === source) { linked.push(agentDir); continue }
...
export async function unlinkFromAgents(skillDirName: string): Promise<void> {
  const source = path.join(HANZO_SKILLS_DIR, skillDirName)
  for (const agentDir of AGENT_SKILL_DIRS) {
    const target = path.join(agentDir, skillDirName)
    try {
...
export async function addSkills(url: string, opts: { force?: boolean } = {}): Promise<{
  dirName: string
  targetDir: string
  count: number
  linked: string[]
...
export async function removeSkills(name: string): Promise<void> {
  ...
  if (!fs.existsSync(targetDir)) {
    ...
    const dirs = (entries as fs.Dirent[]).filter((e) => e.isDirectory() && !e.name.startsWith("."))
...
export async function listSkills(): Promise<Array<{ name: string; path: string; count: number }>> {
